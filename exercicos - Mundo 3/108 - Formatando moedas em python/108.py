...
while numero != 0 :
    ...
    if opçao == 1:
        ...
        print(f'O aumento de {escolha}% de {moeda.moeda(numero)} é: {moeda.moeda(moeda.aumentar(numero,escolha))}\n')
    if opçao == 2:
        escolha = int(input('Em quanto % quer diminuir?'))
        print(f'A redução de {escolha}% de {moeda.moeda(numero)} é: {moeda.moeda(moeda.diminuir(numero,escolha))}\n')
    if opçao == 3:
